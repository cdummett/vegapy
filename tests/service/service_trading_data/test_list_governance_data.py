...
def test_list_governance_data(tds: TradingDataService):
    for governance_data in tds.list_governance_data(max_pages=1):
        assert isinstance(
            governance_data, protos.vega.governance.GovernanceData
        )
...
def test_list_governance_data_proposal_state(
    tds: TradingDataService, proposal_state
):
    ...
    for governance_data in tds.list_governance_data(
        ...
        try:
            ...
            logger.error(
                f"Proposal: {id} | Expected: {expected} | Actual: {actual}"
            )
            raise e
...
def test_list_governance_data_proposal_type(
    tds: TradingDataService, proposal_type
):
    for governance_data in tds.list_governance_data(
