...
def log_client_method(func):
    ...
    def wrapper(*args, **kwargs):
        logger.debug(
            f"Client method '{func.__name__}' called with kwargs: {kwargs}"
        )
...
def unroll_v2_pagination(
    ...
    while response.page_info.has_next_page:
        ...
        logger.debug(
            f"Extended results wih {len(curr_list)} entries from page {page}."
        )
        page += 1
...
class TradingDataService:
    ...
    def switch_node(self):
        for _ in range(100):
            try:
                ...
                logger.debug(
                    f"Unable to open insecure channel with {self.__node}"
                )
                continue
    ...
    def list_accounts(
        ...
        return unroll_v2_pagination(
            base_request=trading_data.ListAccountsRequest(
    ...
    def list_ledger_entries(
        ...
        return unroll_v2_pagination(
            ...
            request_func=lambda x: self.__stub.ListLedgerEntries(
                x
            ).ledger_entries,
            extraction_func=lambda res: [i.node for i in res.edges],
            max_pages=max_pages,
    ...
    @log_client_method
    def get_latest_market_depth(
        self, market_id: str
    ) -> protos.vega.vega.MarketDepth:
    ...
    def get_market_data_history_by_id(
        ...
        return unroll_v2_pagination(
            ...
            request_func=lambda x: self.__stub.GetMarketDataHistoryByID(
                x
            ).market_data,
            extraction_func=lambda res: [i.node for i in res.edges],
            max_pages=max_pages,
    ...
    def list_governance_data(
        ...
        return unroll_v2_pagination(
            ...
            request_func=lambda x: self.__stub.ListGovernanceData(
                x
            ).connection,
            extraction_func=lambda res: [i.node for i in res.edges],
            max_pages=max_pages,
